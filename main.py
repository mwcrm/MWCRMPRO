...
# ── TELEFON KİŞİLER ──────────────────────────────────────────────────────────
    ...
    with tab_rehber1:
        ...
        if df_kis.empty:
            ...
            h1,h2,h3,h4,h5,h6,h7 = st.columns([2,2,1.5,1.5,2,0.8,0.8])
            for hdr,txt in zip([h1,h2,h3,h4,h5,h6,h7],["Ad Soyad","Firma","Tel","Mesaj","Şablon","📱","⚙️"]):
                hdr.caption(f"**{txt}**")
            st.divider()
            ...
            for _, kisi in df_kis.iterrows():
                ...
                c1,c2,c3,c4,c5,c6,c7 = st.columns([2,2,1.5,1.5,2,0.8,0.8])
                c1.markdown(f"**{isim}**")
                c2.caption(firma_k[:20] if firma_k else "—")
                c3.caption(tel[:12] if tel else "—")
                ...
                if tel:
                    ...
                    sec = c5.selectbox("", opts, key=f"ks_{_kisi_id}", label_visibility="collapsed")
                    mesaj_gonder = ""
                    if sec == "✏️ Manuel":
                        mesaj_gonder = st.text_area("", height=50, key=f"km_{_kisi_id}", placeholder=f"Merhaba {isim}...", label_visibility="collapsed")
                        ...
                        if not match.empty:
                            ...
                            mesaj_gonder = st.text_area("", value=sab_metin, height=50, key=f"km_{_kisi_id}", label_visibility="collapsed")
    ...
    with tab_rehber5:
        ...
        if df_mlog_all.empty:
            ...
            if "sablon_adi" in df_mlog_all.columns:
                ...
                sab_oz.columns = ["Şablon Adı","Kullanım","Farklı Kişi","Son Gönderim"]
